...
class CppNativeBinariesPluginTest extends Specification {
    ...
    def "creates cpp source set with conventional locations for components"() {
        ...
        sourceSets.exe.cpp.precompiledHeaders.srcDirs == [] as Set
        ...
        sourceSets.lib.cpp.precompiledHeaders.srcDirs == [] as Set
    ...
    def "creates compile tasks for each executable source set"() {
        ...
        binary.tasks.withType(CppPrecompileHeader)*.name == ["precompileHeaderTestExecutableTestAnotherCpp", "precompileHeaderTestExecutableTestCpp"]
        ...
        binary.tasks.withType(CppCompile).each { compile ->
            ...
            compile.taskDependencies.grep{ it instanceof CppPrecompileHeader }.size() == 1
        }

        and:
        binary.tasks.withType(CppPrecompileHeader).each {
            it.toolChain == binary.toolChain
            it.macros == [NDEBUG:null, LEVEL:"1"]
            it.compilerArgs == ["ARG1", "ARG2"]
        }
    ...
    def "creates compile task for each library source set"() {
        ...
        sharedLib.tasks.withType(CppPrecompileHeader)*.name == ["precompileHeaderTestSharedLibraryTestAnotherCpp", "precompileHeaderTestSharedLibraryTestCpp"]
